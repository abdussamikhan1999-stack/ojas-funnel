...
- **The site is now live**: manually imported via the Vercel dashboard (Import Git Repository) into a project git-connected to this repo, at `https://abdussamikhan1999-stack-ojas-funnel.vercel.app/`. Verified serving the real content with clean URLs working (`/quiz`, `/product`, etc.). The Vercel MCP connector could not do this reliably in-session — see the note below — so the dashboard is the trusted path until that's fixed.
- **Vercel MCP connector caution**: in one session, `deploy_to_vercel` (manual file-tree deploy) allowed **exactly one deployment per project, ever** — every second call to an already-deployed project failed with a 403 "no permission to create a Production Deployment," regardless of project. `create_git_project` and `get_project`/`list_projects`/`get_git_deployment_context` were also mutually inconsistent (writes referenced projects that reads couldn't see). Don't trust that connector's reads, and don't expect more than one shot at a manual deploy per project — use the dashboard's Import Git Repository flow instead, which builds from the whole repo correctly.
